...
- First run auto-creates `~/.pi/proxy-domains.json` from the example content (never overwrites): whitelist preloaded, `proxy` left empty for the fallback chain; startup TIP prompts to fill it in
